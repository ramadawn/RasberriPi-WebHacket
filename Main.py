...
def retrieve_network():
    ...
    #preform scan of network
    open_ip_list, available_ip_list, vendors = ImprovedScanner.ImprovedScanner(int(port),127, network=network)
    ...
    for idx, ip in enumerate(available_ip_list):
        #storage variable for open port flag
        openPort = False
        if(ip in open_ip_list):
            openPort = True
            
        output = f"IP: {ip}      Port {port} open = {openPort}    Vendor: {vendors[ip]}"
